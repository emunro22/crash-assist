...
import { getNearbyAreas } from '@/lib/areas-data'
import { nearMeAngles, getAngleBySlug } from '@/lib/near-me-data'
import { getServiceBySlug } from '@/lib/services-data'

export default function NearMeAreaPage({ area, angleSlug }: { area: Area; angleSlug: string }) {
  const angle = getAngleBySlug(angleSlug)!
  const data = angle.build(area)
  const nearby = getNearbyAreas(area, 4)
  const relatedService = getServiceBySlug(data.relatedServiceSlug)
  const otherAngles = nearMeAngles.filter((a) => a.slug !== angle.slug)
      ...
      <section className="relative pt-32 pb-20 bg-zinc-950 overflow-hidden">
        ...
        <div className="container relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Link href={`/areas/${area.slug}`} className="inline-flex items-center gap-2 text-zinc-500 hover:text-blue-500 text-sm mb-8 transition-colors">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
              {area.name} Coverage
            </Link>
            <div className="section-tag mb-4">{area.name}</div>
        ...
        <div className="container">
          <div className="grid lg:grid-cols-[2fr_1fr] gap-16">
            <div>
              ...
              <div className="mt-12">
                <h2 className="font-heading text-3xl font-black text-white uppercase mb-6">
                  {area.name} <span className="text-blue-500">FAQs</span>
                </h2>
            ...
            <div className="space-y-6">
              ...
              {/* Related service + full area page */}
              <div className="bg-zinc-900 border border-zinc-800 p-6">
                <h3 className="font-heading font-bold text-white uppercase text-sm mb-4">More About {area.name}</h3>
                <ul className="space-y-2">
                  <li>
                    <Link href={`/areas/${area.slug}`} className="text-zinc-400 hover:text-blue-500 text-sm transition-colors flex items-center gap-2">
              ...
              {/* Other angles for this area */}
              <div className="bg-zinc-900 border border-zinc-800 p-6">
                <h3 className="font-heading font-bold text-white uppercase text-sm mb-4">Other Searches in {area.name}</h3>
                <ul className="space-y-2">
                  {otherAngles.map((a) => (
                    <li key={a.slug}>
                      <Link href={`/areas/${area.slug}/${a.slug}`} className="text-zinc-400 hover:text-blue-500 text-sm transition-colors flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-blue-500" />
                        {a.navLabel}
                      </Link>
                    </li>
              ...
              {/* Nearby areas, same angle */}
              {nearby.length > 0 && (
                <div className="bg-zinc-900 border border-zinc-800 p-6">
                  <h3 className="font-heading font-bold text-white uppercase text-sm mb-4">{angle.navLabel} Nearby</h3>
                  <ul className="space-y-2">
                    {nearby.map((n) => (
                      <li key={n.slug}>
                        <Link href={`/areas/${n.slug}/${angle.slug}`} className="text-zinc-400 hover:text-blue-500 text-sm transition-colors flex items-center gap-2">
                          <span className="w-1 h-1 rounded-full bg-blue-500" />
                          {n.name}
                        </Link>
                      </li>
