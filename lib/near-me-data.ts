...
// through all 14 areas in lockstep, avoiding a fully mail-merged feel.
    ...
    return {
      ...
      faqs: [
        ...
        },
        {
          ...
          answer: "We can't promise the exact minute, since traffic and current jobs both factor in, but we give you an honest ETA on the call and update you if anything changes.",
        },
      ],
      relatedServiceSlug: 'breakdown-recovery',
      relatedServiceLabel: 'Breakdown Recovery',
...
const hour24RecoveryNearMe: NearMeAngle = {
  ...
  build: (area) => {
    ...
    const intros = [
      ...
      `Whatever time you've broken down in ${area.name}, whether it's the middle of the night, a bank holiday, or Christmas Day, the phone is answered and a truck is arranged. That's what "24 hour" is supposed to mean.`,
    ]
    return {
...
const cheapRecoveryNearMe: NearMeAngle = {
  ...
  build: (area) => {
    ...
    const v = variant(area.slug, 3)
    const intros = [
      `"Cheap" usually means hidden fees waiting on arrival. Ours doesn't. Recovery in ${area.name} is quoted clearly on the phone before we dispatch, so there are no surprises when the truck turns up.`,
