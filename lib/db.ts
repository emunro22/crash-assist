...
function getSql() {
  ...
  if (!url) throw new Error('DATABASE_URL is not set. Add a Neon Postgres database in Vercel → Storage.')
  _sql = neon(url)
  return _sql
}
...
export async function getGalleryImages(): Promise<GalleryImage[]> {
  ...
  void rows
  return (await sql`
    SELECT id, url, title, tag, blob_path, created_at FROM gallery_images ORDER BY created_at DESC
  `) as GalleryImage[]
}

export async function addGalleryImage(data: {
  url: string; title: string; tag: string; blob_path?: string
}): Promise<GalleryImage> {
  await sql`
    CREATE TABLE IF NOT EXISTS gallery_images (
      id SERIAL PRIMARY KEY, url TEXT NOT NULL, title TEXT NOT NULL DEFAULT '',
      tag TEXT NOT NULL DEFAULT 'General', blob_path TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `
  const rows = (await sql`
...
export async function addBlogPost(data: {
  ...
  cover_image_url?: string | null; cover_image_path?: string | null
  ...
  const rows = (await sql`
    ...
    VALUES (${data.title}, ${data.slug}, ${data.excerpt}, ${data.content},
            ${data.cover_image_url ?? null}, ${data.cover_image_path ?? null},
            ${data.category}, ${data.published})
...
export async function getCustomers(): Promise<Customer[]> {
  ...
  return (await sql`
    ...
    FROM customers ORDER BY job_date DESC, created_at DESC
  `) as Customer[]
}

export async function addCustomer(data: {
  name: string; phone: string; job_date: string; notes?: string; source?: 'manual' | 'whatsapp'
}): Promise<Customer> {
  await ensureCustomerTable()
  const rows = (await sql`
...
export async function upsertWhatsAppCustomer(data: {
  phone: string; name: string; job_date: string
}): Promise<{ customer: Customer; isNew: boolean }> {
  await ensureCustomerTable()
  const month = data.job_date.slice(0, 7)
  const existing = (await sql`
    SELECT id, name, phone, job_date::TEXT, notes, source, created_at
    FROM customers WHERE phone = ${data.phone} AND TO_CHAR(job_date, 'YYYY-MM') = ${month} LIMIT 1
  `) as Customer[]
...
export async function getCustomersByMonth(yearMonth: string): Promise<Customer[]> {
  ...
  return (await sql`
    SELECT id, name, phone, job_date::TEXT, notes, source, created_at
    FROM customers WHERE TO_CHAR(job_date, 'YYYY-MM') = ${yearMonth}
